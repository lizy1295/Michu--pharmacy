...
import { getDoctorById, updateDoctor, uploadDoctorImage, getImageUrl } from '@/lib/api/admin';
...
export default function AdminDoctorsEditPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
    ...
    const fetchDoctor = async () => {
      try {
        ...
        setFormData({
          ...
          availableForConsultation: doctor.availableForConsultation ?? true,
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500 font-semibold">Loading doctor data...</div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900">Edit Doctor</h1>
          <p className="text-sm text-slate-500 mt-0.5">Update profile for Dr. {formData.firstName} {formData.lastName}</p>
        </div>
      </div>
